...
const BookDetail = () => {
  ...
  const clickSave = (id: number) => {
    const savedBooks = JSON.parse(localStorage.getItem("savedBooks") || "[]");

    if (savedBooks.includes(id)) {
      localStorage.setItem(
        "savedBooks",
        JSON.stringify(savedBooks.filter((bookId: number) => bookId !== id)),
      );
    } else {
      localStorage.setItem("savedBooks", JSON.stringify([...savedBooks, id]));
    }
  };
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-auto">
                  ...
                  <button
                    ...
                    className={`h-12 px-6 border-2 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                      saved
