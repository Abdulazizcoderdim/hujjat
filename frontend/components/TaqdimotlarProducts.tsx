...
async function getTaqdimotlar() {
  const params = new URLSearchParams({
    limit: "4",
    categoryId: "69579aea425b3c86947ed71f",
  });
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[Taqdimotlar] API ${res.status}: ${res.statusText}, URL: ${url}`);
      return {
        items: [],
    ...
    console.error("[Taqdimotlar] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const TaqdimotlarProducts = async () => {
  const data = await getTaqdimotlar();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 text-primary mb-2">
              ...
              <span className="text-sm font-medium">Taqdimotlar</span>
            </div>
