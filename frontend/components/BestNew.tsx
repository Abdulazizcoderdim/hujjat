...
async function getBestNewProducts() {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[BestNew] API ${res.status}: ${res.statusText}, URL: ${url}`);
      return {
        items: [],
    ...
    console.error("[BestNew] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const BestNewProducts = async () => {
  const data = await getBestNewProducts();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 text-accent mb-2">
              ...
              <span className="text-sm font-medium">Tavsiya etilgan</span>
            </div>
