...
async function getMustaqil() {
  const params = new URLSearchParams({
    limit: "4",
    categoryId: "695792b62804ee23a8561f83",
  });
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[Mustaqil] API ${res.status}: ${res.statusText}, URL: ${url}`);
      return {
        items: [],
    ...
    console.error("[Mustaqil] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const MustaqilProducts = async () => {
  const data = await getMustaqil();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 text-accent mb-2">
              ...
              <span className="text-sm font-medium">Mustaqil ishlari</span>
            </div>
