...
async function getDiplom() {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `[Diplom] API ${res.status}: ${res.statusText}, URL: ${url}`,
      );
      return {
        items: [],
    ...
    console.error("[Diplom] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const DiplomProducts = async () => {
  const data = await getDiplom();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 text-accent mb-2">
              ...
              <span className="text-sm font-medium">Diplom ishlari</span>
            </div>
