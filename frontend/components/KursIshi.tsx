...
async function getKursIshi() {
  const params = new URLSearchParams({
    limit: "4",
    categoryId: "69579af0425b3c86947ed722",
  });
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[KursIshi] API ${res.status}: ${res.statusText}, URL: ${url}`);
      return {
        items: [],
    ...
    console.error("[KursIshi] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const KursIshlariProducts = async () => {
  const data = await getKursIshi();
