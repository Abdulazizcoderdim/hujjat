...
async function getTestlar() {
  const params = new URLSearchParams({
    limit: "4",
    categoryId: "69579ae1425b3c86947ed719",
  });
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[Testlar] API ${res.status}: ${res.statusText}, URL: ${url}`);
      return {
        items: [],
    ...
    console.error("[Testlar] Fetch xatolik:", error);
    return {
      items: [],
      pagination: { total: 0, page: 1, limit: 10, totalPages: 1 },
    };
...
export const TestlarProducts = async () => {
  const data = await getTestlar();
